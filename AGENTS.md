...
**Ordre de construction et porte de publication.** L'ordre arrêté est 26 → 24 → 25 → 28 → 30 → 27, détaillé et argumenté dans [docs/positionnement.md](docs/positionnement.md) ; les tâches 17 à 23, 37 et 38 se traitent en parallèle. **La 26 et la 24 sont livrées** (12 septembre 2026). La 26 : la première validation par un agent réel a prouvé la découverte des skills, la délégation des sous-agents, le déclenchement des hooks et la lecture d'`AGENTS.md`, et corrigé au passage un défaut que la suite automatisée ne voyait pas (l'en-tête généré devant le frontmatter en mode copie) ; le scénario est versionné dans [e2e/validation-agent/](e2e/validation-agent/) et se rejoue après chaque release. La 24 : le méta-skill `$propose-setup` analyse le dépôt et présente une proposition d'ensemble — hooks, règles, sous-agents — chacune avec sa raison et ses preuves, acceptable ou refusable ligne à ligne, de sorte que l'`init` ne rend plus un squelette vide. **Porte restante avant publication** : le critère « un inconnu installe en moins de cinq minutes en lisant le seul README » doit avoir été vérifié pour de vrai.
...
- `.agents/tasks/` — backlog : 17 à 23 (dette technique issue du premier audit), 25, 27, 28, 29 et 30 (produit), 37 et 38 (issues de la validation par un agent réel du 12 septembre 2026), 39 (maintenance activable de la configuration, périmètre à trancher). L'ordre de prise est celui de `docs/positionnement.md`, pas l'ordre des numéros ; il reste 25 → 28 → 30 → 27.
