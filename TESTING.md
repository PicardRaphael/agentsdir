...
| Chaque générateur dit ce qui est source et ce qui est dérivé, et `add hook` énonce la portée du hook | `src/commands/__tests__/generator-next-steps.test.ts` | les clauses sur `openai.yaml`/`icon.svg`, sur les harness sans projection, et l'étape `narrow it with a matcher` — **action comprise** : n'asserter que la justification laissait passer un renommage de l'action |
| Chaque générateur nomme l'étape suivante, et nomme un **chemin** plutôt qu'un contenu | `src/commands/__tests__/generator-next-steps.test.ts` | le bloc `nextSteps` de chacun, et le chemin dans sa première action — le dépôt porte la source et sa projection sous deux noms |
| Le bloc ne s'imprime ni en `--dry-run` ni pour `pack add`, qui partage le même rendu | `src/commands/__tests__/generator-next-steps.test.ts` | la garde `options.dryRun ? [] : …` et le caractère optionnel de `nextSteps` (`src/commands/add-common.ts`) |
| L'invocation affichée se dérive des harness activés, jamais d'un nom écrit en dur | `src/commands/__tests__/generator-next-steps.test.ts` | `invocationStep`, et le champ `invocation` de `HARNESS_SPECS` — `undefined` pour un harness dont aucune syntaxe n'a été observée |
| Un registre de hooks préexistant est annoncé `updated`, pas `created` | `src/commands/__tests__/generator-next-steps.test.ts` | `raw === undefined ? "created" : "updated"` (`src/core/hook-registries.ts`) — mutation à valider après `npm run build`, le test passe par un sous-processus |
...
Au 13 septembre 2026 : **86 % des instructions, 76 % des branches**.

**Le plafond est là, et il est assumé.** Ce qui reste non couvert est de quatre natures, chacune tranchée ci-dessous : le câblage citty, les questions `@clack/prompts`, les sorties console et les branches défensives. Chaque point au-delà exigerait de tester un mock ou un import — c'est-à-dire d'écrire des tests qui n'affirment rien, pour contredire les décisions de ce document. 100 % n'est pas un objectif ici ; ce qui en est un, c'est qu'aucune ligne que l'utilisateur lit ou heurte ne reste sans test, et c'est fait.

**Une voie a été explorée puis écartée, et il vaut la peine de dire pourquoi.** 180 des 452 instructions non couvertes sont le câblage citty et `src/cli.ts`, déjà traversés à chaque exécution par les tests du contrat `--json` et par `e2e/` — mais dans un processus fils que la couverture v8 n'instrumente pas. Les faire compter est techniquement faisable : `NODE_V8_COVERAGE` sur les fils, un bundle de test portant `sourcemap: true` (jamais celui qui est publié), et `v8-to-istanbul` pour remonter du bundle aux sources. Le prototype fonctionne — `cli.ts` passe de 0 % à 89 % avec deux invocations.

Ce qui l'arrête n'est pas le coût mais **l'incompatibilité des deux mesures**. Le remappage du bundle compte les instructions à la granularité de V8, vitest les compte à celle de l'AST TypeScript : `src/commands/sync.ts` vaut 581 instructions d'un côté et 152 de l'autre. Fusionner les deux jeux produirait un pourcentage unique qui n'aurait aucun sens, et publier deux chiffres pour un même fichier rendrait le tableau moins lisible qu'un blanc assumé. La décision est donc de garder une seule mesure, celle des tests en mémoire, et d'écrire ici que le câblage citty est couvert **ailleurs** — par les sous-processus — plutôt que de le faire apparaître dans un chiffre bâtard.

Les 272 instructions restantes, elles, sont le vrai mur : questions `@clack/prompts` et branches défensives.
...
| `src/commands/update.ts` | 87 % | **Couvert le 13 septembre 2026.** Les chemins où le lock contredit le disque — lock illisible, dossier ou fichier verrouillé disparu, entrée d'un pack que le dépôt ne déclare plus — sont dans `update-lock-paths.test.ts` ; `renderUpdateReport` et `mergeChanges` — le texte que l'utilisateur lit et la règle de fusion qui le nourrit — ont leurs tests directs (`src/commands/__tests__/reports.test.ts`), branche d'abandon et conflits non tranchés compris. Reste `askResolver` (prompts, assumé comme `init-interview`) et le bloc citty `run()` (exercé en sous-processus). |
...
- **Deux règles de `check` sur cinquante-trois restent hors de portée d'un test**, et c'est une limite de l'environnement, pas un oubli (état au 13 septembre 2026 ; le trou de quatre règles noté en août est comblé). `hook-protocol-crash` exige que `spawn` échoue à démarrer `node` lui-même — il n'y a pas de point d'injection, et en ouvrir un dans le code de production pour gagner une ligne de couverture serait ouvrir une surface publique pour une mesure. `agent-unreadable` exige un fichier `.md` présent dans `.agents/agents/` que `readFile` refuse : seule une permission le produit, et `chmod` est inopérant sous Windows où la CI tourne aussi. La technique d'échange de nature — un répertoire à la place du fichier — fait sortir l'entrée du filtre `isFile()` et la règle n'est pas atteinte ; `check-diagnostics.test.ts` enregistre ce fait plutôt que de laisser croire à une couverture. Les cinquante et une autres sont nommées par un test qui lit leur message.
...
- **Les sorties `console.log` / `console.error` et `process.exit`** : ce que la CLI écrit sur stdout et le code qu'elle rend sont vérifiés là où ils comptent — en sous-processus, par les tests du contrat `--json` et par `e2e/`. Les appeler en mémoire demanderait d'intercepter `console`, c'est-à-dire de vérifier l'interception.
...
- **La garde `NAME_SPEC` de la boucle de lock d'`update` (`src/commands/update.ts`)** : `validateRepo` refuse une clé qui n'est pas un nom de dossier et arrête la commande avant cette boucle, donc la seconde vérification n'est jamais observable. Elle reste — la boucle utilise la clé comme segment de chemin, et un appelant futur pourrait ne pas valider d'abord — mais aucun test ne peut la prouver, seulement le refus en amont, qui lui est testé.
