...
**Une voie reste ouverte, et elle est honnête : instrumenter les sous-processus.** 180 des 452 instructions non couvertes sont le câblage citty et `src/cli.ts`, déjà traversés à chaque exécution par les tests du contrat `--json` et par `e2e/` — mais dans un processus fils que la couverture v8 n'instrumente pas. Les faire compter (`NODE_V8_COVERAGE` et une configuration tsup de test portant `sourcemap: true`, pour ne pas ajouter de `.map` au paquet publié) porterait la mesure à **91,6 %** sans écrire une ligne de test : le code est déjà exercé, seule la mesure lui échappe. Ce n'est pas fait ici — c'est de l'outillage, décidable à froid. Les 272 instructions restantes, elles, sont le vrai mur : questions `@clack/prompts` et branches défensives.
