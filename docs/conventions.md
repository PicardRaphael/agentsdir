...
## 13. The dated norms, and the age of the CLI

### Why the norms are data

Practices move faster than repositories. A repository configured six months ago
carries a shape that was a good practice and is no longer one, and nothing says
so. `check` answers a different question — is this configuration still in step
with its source — and answering it well does not answer this one at all.

So the norms this CLI applies are **data**, in `src/core/conventions-dates.ts`,
each carrying its bound, its source and the day that source was read. None of
them is a number typed into a condition: `core/context-budget` and
`core/validate` read their bounds from that table, and a bump there moves the
report, the invariant and the shipped reference together.

The dating convention is the one `core/hook-registries` already applies: the
Agent Skills specification publishes **neither a version number nor a
changelog**, so a norm cannot be dated by its source. It is dated at the day it
was **read**, with what was read. An undated bound is an opinion.

### The norms shipped by this version
...
What is **not** in this table: the thresholds agentsdir chose for its own
catalog — a body of at least 12 significant lines, a `short-description` of 25
to 64 characters, `#RRGGBB` colors, the name grammar. They are this project's
decisions (§4), they have no outside source, and dating them would lend them an
authority they have not got.

### The age of the CLI, computed offline

`doctor` opens its report with two lines: the version of the CLI and the day it
was cut, and the day the embedded norms were read. Past **six months**, each
turns into an explicit notice naming the age and what to do about it.

The conventions age by the **oldest** consultation in the table, never the most
recent: a table refreshed in one row and stale in six is as old as its oldest
row.

Both dates are constants of the package (`src/version.ts`,
`core/conventions-dates.ts`), set by hand at release time. Neither is computed
at build time — every generated byte of this CLI is reproducible, and a build
date would change the bundle at each build — and neither is fetched. Asking the
npm registry for the published version would send the package name out of the
repository on an ordinary command; if that option is ever added it will sit
behind an explicit flag, never by default, and never in `check`.

**No command of this CLI opens a network connection.** Two tests hold it: one
lays a trap — `fetch`, the two DNS entry points and `Socket.prototype.connect`
all throw — and runs `doctor`, `check` and the context budget through it; the
other reads every source file of `src/` and fails on any network API, which
covers the code no run reaches.

### The four diagnostics, and the one that is a judgement

A configuration can be asked four questions. They are independent, they have
different answers, and mixing them produces an unusable report:

| Question | Answered by | Fails CI |
| --- | --- | --- |
| Has it **drifted** from its source? | `check` (§4) | yes |
| Does it **serve**? | `$review-usage` (§11) | no |
| What does it **cost** in context? | `doctor` (§10) | no |
| Is its **form** still the one recommended? | `$review-form` | no |

The last one is an appreciation, not a measurement — "these five skills would be
better merged" is an argument, not a count. So it is a **meta-skill of the
`creator` pack**, never a condition in the CLI, exactly as the split between
the deterministic and the arguable runs everywhere else here.

`$review-form` reads the norms from
`.agents/skills/review-form/references/conventions.md`, which `sync` renders
from the table above — the agent quotes a dated norm instead of recalling one.
Every proposal carries the norm, its source, the day it was read and the
evidence found in the repository; a proposal missing any of the three is
deleted. It answers the form question only, sends the other three to their own
commands, writes its report into `.agents/output/form/` (git-ignored), and
**applies nothing**: the tool proposes, the team decides.
