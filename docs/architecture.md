...
| `core/manifest` | Reading, validation and writing of the `.agents.toml` manifest. `planManifest` renders it and says what a write would do; `applyManifestPlan` is the only function that writes it, atomically. | The only module that writes the manifest — enforced by a test, after four commands had each rendered and written it themselves. They drifted: one rebuilt the manifest field by field and dropped the `[usage]` section on every sync, taking a user's `enabled = false` and their privacy globs with it. Carrying state forward is a spread, never a list of fields. |
...
| `core/conventions-dates` | The outside norms this CLI applies, as data: each carries its bound, its source and the day that source was read (see [conventions.md](conventions.md) §13). `core/context-budget` and `core/validate` read their bounds from it, `doctor` reports its age, and the `review-form` meta-skill is shipped with it rendered as a reference. | The specification publishes no version number, so a norm is dated at its **consultation**, not by its source — the convention `core/hook-registries` already applies. Nothing here is fetched: the norms travel by npm with the version of the CLI, and no command opens a network connection. The thresholds agentsdir chose for its own catalog stay out of the table: they have no outside source, and dating them would lend them an authority they have not got. |
| `core/context-budget` | What the installed configuration costs in context, and **when** it is paid: always, on invocation, or when relevant (see [conventions.md](conventions.md) §10). Bytes and lines exact, tokens estimated against a calibrated divisor. | Read-only and toothless on purpose: the report goes to `doctor`, and an overrun of an Agent Skills bound is information — `check` stays the guardian of drift, not of sobriety. |
| `core/usage-journal` | The journal's location and retention window, shared by the two stages of the `usage` pack: the collectors write and prune there, the review reads and states the window as a limit. | A path spelled twice is a guard watching a directory nobody writes to. |
...
| `core/lock` | `skills-lock.json`: provenance and sha256 fingerprint of vendored skills (sorted relative paths, content included, `.git` and `node_modules` excluded), plus the `files` table tracking the generic rules and shared scripts the packs install outside any skill folder. One planner — read, shape check, delta, re-lock, render — and one writer, for `init`, `pack add|remove`, `sync` and `update` alike. | Detects local drift in an imported skill or an installed rule; it does not download anything itself. Only what an install wrote is tracked — a file kept as the repository had it is never pinned to a render it did not receive. The asymmetry the callers must not re-decide: an `agentsdir` entry stays pinned to the installed version (recomputing it would bless a local edit and lose the `update` protection), a `github` entry is re-locked to the folder on disk. |
...
| `core/mcp` | The MCP servers declared once in `.agents/mcp.toml`, and the plan that projects them into `.mcp.json`, `.cursor/mcp.json` and the managed block of `.codex/config.toml` (see [conventions.md](conventions.md) §12 and [harness.md](harness.md) §4). | The twin of the hook registries, with two deliberate differences. Ownership is by **name**, not by content: a server whose `args` changed would otherwise become untouchable and stay configured for good, so `[mcp] servers` in the manifest records what was projected and makes removal possible. And the source carries **names of environment variables**, never values — `.agents/` is committed, and each harness resolves a name its own way at run time. |
...
| `core/fs-utils` | The filesystem probes shared by every layer: `pathExists`, `entryExists` (a broken symlink still counts), `isDirectory`, `writeFileAtomic` — used by **every** command that writes, generators included — and `walkFiles` — the single recursive walk, sorted by name in code units, parameterised by its exclusions and by what an unreadable root means to its caller. | Absence is a normal answer in this CLI, never an exception re-caught at each call site. The walk order is a contract, not a detail: the `skills-lock.json` fingerprints are fed in it, so a locale-aware sort would invalidate every entry on another machine. |
...
- **`CLAUDE.md` in copy mode: a bridge, not a copy** (decision recorded in task 05). The generated file contains the `@AGENTS.md` import followed by the generated header: a single source is read by Claude Code whatever the mechanism (symlink or import). Empirical validation: this repo's bridge `CLAUDE.md` has worked this way since the first commit. The alternative "full copy of the content with a header" was ruled out: duplicated content, guaranteed drift between two `sync` runs. In directory copies (`.claude/{rules,skills,agents}`), the header is only placed on Markdown files; other files are copied byte for byte. A Markdown file that opens with a YAML frontmatter block keeps that block first and takes the header right after it: a harness only parses frontmatter that starts on the very first byte, and prefixing the header made Claude Code drop a sub-agent from the delegation list and read a skill without its metadata while `check` stayed green (validation of 2026-09-12, see [../e2e/validation-agent/](../e2e/validation-agent/)).
